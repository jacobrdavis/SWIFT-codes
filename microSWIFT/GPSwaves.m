...
%               
%  J. Davis,   12/2021,  J. Davis added the "EJ" time domain filter
%
%#codegen
...
%%% Jake modified point limit: 2*round(fs*wsecs)
if pts >=  2*round(fs*wsecs) & fs>=1 & sum(badu)<100 & sum(badv)<100,  % minimum length and quality for processing
...
% 
% 
% %%% Q: Opportunity for vectorization here?
% for ui = 2:length(z)
...
%%% Q: Why is the bandwidth not (Nyquist-1/wsec)/n? NVM...it is
...
figure
semilogy(f,UUwindowmerged/(win/2*fs),'color',[0.5,0.5,0.5]); hold on
semilogy(f,UU,'r','linewidth',2)
figure
semilogy(f,VVwindowmerged/(win/2*fs),'color',[0.5,0.5,0.5]); hold on
semilogy(f,VV,'r','linewidth',2)
figure
semilogy(f,ZZwindowmerged/(win/2*fs),'color',[0.5,0.5,0.5]); hold on
semilogy(f,ZZ,'r','linewidth',2)
...
%Q: this is not even close to 1 though?
figure
plot(f,check)
